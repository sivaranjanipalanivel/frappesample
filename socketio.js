...
io.on('connection', function(socket) {
	...
	// socket.on('order_notification',function(delay,order_id){
	// 	// socket.broadcast.to(data.room).emit('watch_for_notification', data)
	// 	if(delay){
	// 		set_timeout_for_notification(delay,order_id)
	// 		// setTimeout(function(){
	// 		// 	console.log('delay of '+data.delay+' seconds is completed for order '+data.order_id)
	// 		// 	socket.emit('ecommerce_business_store.ecommerce_business_store.api.check_order_notification',{'order':data.order_id})
	// 		// },delay*1000)
	// 	}
	// });
	socket.on('init_notification',function(data){
		console.log('---------socket server---------------')
		if(data.order_id){
			// io.socket.post('/api/method/ecommerce_business_store.ecommerce_business_store.api.check_order_notification',data,function(res,r){

			// });
			io.socket.request({
				method:'post',
				url:'/api/method/ecommerce_business_store.ecommerce_business_store.api.check_order_notification',
				data:data,
				async:false,
				headers:{
...
function set_timeout_for_notification(delay,order_id){
	...
	var init_timeout=setTimeout(function(){
		...
		$.ajax({
            ...
            url: window.location.origin + '/api/method/ecommerce_business_store.ecommerce_business_store.api.check_order_notification',
            data: { 'order': order_id },
            dataType: "json",
            async: false,
            headers: {
