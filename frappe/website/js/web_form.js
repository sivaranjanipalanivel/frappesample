frappe.ready(function() {
			...
			if(input.files.length) {
				...
				reader.onload = function(e) {
					...
					if(input.filedata.dataurl.length >
						...
						$('.upload-content .file_url').text(file.name)
                    	$('.upload-content .drop-txt').hide()
                    	$('.upload-content .max-size').hide()
					}
					frappe.file_reading = false;
				}
		...
		frappe.call({
			...
			callback: function(data) {
				if(!data.exc) {
					...
					if(frappe.web_form_name=='customer-registration'){
						login_buyer(frappe.doc_name)						
					}
					...
					if(frappe.web_form_name=='customer-profile'){
						let message='Your profile updated successfully.';
						frappe.msgprint(message,'Success')
						setTimeout(function(){
							window.location.href = window.location.pathname + "?name=" + frappe.doc_name;
						},1000)
					}
	...
	function login_buyer(customer){
		$.ajax({
			type: 'POST',
            Accept: 'application/json',
            ContentType: 'application/json;charset=utf-8',
            url: window.location.origin + '/api/method/ecommerce_business_store.ecommerce_business_store.api.get_customer_info1',
            ...
            success: function(data){
            	if(data.message){
            		$.ajax({
				        ...
				        Accept: 'application/json',
				        ContentType: 'application/json;charset=utf-8',
				        url: window.location.origin + '/api/method/login?usr=' + data.message.user_id + '&pwd=' + data.message.new_password,
				        dataType: "json",
				        async: false,
			            success: function(data){
			            	$('#message_modal .modal-title').text('Success');
				            $('#message_modal .modal-body').html('You have been successfully registered!')
				            $('#message_modal').modal('show');
				            var url=window.location.href;
				            var return_url='';
				            if(url.includes('?')){
								return_url=url.split('?')[1].split('=')[1];								
							}else{
								return_url='edit-profile?name='+customer
							}
				            setTimeout(function(){ window.location.href='/'+return_url }, 3000);
			            }
            		})
            	}
            }
