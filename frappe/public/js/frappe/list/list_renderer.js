...
frappe.views.ListRenderer = Class.extend({
    ...
    set_columns: function() {
        ...
        var docfields_in_list_view =
            frappe.get_children('DocType', this.doctype, 'fields', { 'hidden': 0, 'in_list_view': 1 })
            .sort(function(a, b) {
                return a.idx - b.idx
            });
    ...
    add_column: function(df) {
        ...
        var col;
        var str = df.depends_on
        // var str = "This is the string where we will check if our word exists.";

        if (str) {
            if (str.includes("frappe.boot.active_domains") == true) {
                var basest = str.split("eval: ")
                if (basest[1]) {
                    var basepre = basest[1].split('{{')
                    if (basepre[1]) {
                        var base = basepre[1].split('}}')
                        // console.log(eval(base[0]))
                        if (eval(base[0])) {
                            col = {
                                colspan: colspan,
