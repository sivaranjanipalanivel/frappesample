frappe.socketio = {
	...
	init: function() {
		...
		// publish realtime for orders
		frappe.socketio.socket.on('order_tracking', function(data) {
			if(window.location.pathname=='/thankyou' || window.location.pathname=='/track-order'){
				organize_data(data)
			}			
		});
		frappe.socketio.socket.on('new_driver_assign', function(data) {
			
		});
		frappe.socketio.socket.on('watch_for_notification',function(data){
			// console.log(data)		
			// if(data.delay){
			// 	set_timeout_for_notification(data.delay,data.order_id)
			// }
			frappe.socketio.socket.emit('order_notification',data.delay,data.order_id)
		})
		frappe.socketio.socket.on('check_for_drivers',function(data){
			console.log(data)
			if(data.delivery){
				check_drivers(data.delivery)
			}
		})		
		// watch_for_notification()
	},
	get_host: function() {
...
function set_timeout_for_notification(delay,order_id){
	console.log('initialized socket for order '+order_id)
	var init_timeout=setTimeout(function(){
		console.log('notification will be triggered for order '+order_id+' after '+delay+' secs')
		frappe.call({
			method:'ecommerce_business_store.ecommerce_business_store.api.check_order_notification',
			args:{
				order:order_id
			},
			callback:function(r){
...
function check_drivers(delivery){
	var interval=setInterval(check_notification,1000)
	function check_notification(){
		console.log('executing function for every 1 seconds')
		frappe.call({
			method:'ecommerce_business_store.ecommerce_business_store.doctype.order_delivery.order_delivery.check_for_notification',
			args:{
				name:delivery
			},
